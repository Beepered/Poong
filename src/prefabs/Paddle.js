class Paddle extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, playerID) {
    if (playerID == 1) {
      //left paddle
      super(scene, 50, gameHeight / 2, "paddle");
      this.cooldown_text = scene.add
        .bitmapText(this.x - 8, this.y, "Pixel", "100%", 8)
        .setOrigin(1, 0.5);
    } else {
      //right paddle
      super(scene, gameWidth - 50, gameHeight / 2, "paddle");
      this.tint = 0x000000;
      this.cooldown_text = scene.add.bitmapText(
        this.x + 8,
        this.y,
        "Pixel",
        "100%",
  ...
  update(time, delta) {
    ...
    if (playing) {
      ...
      if (this.cooldown > 0) {
        ...
        if (this.cooldown < 0) {
          //just to help with cooldown percentage text
          this.cooldown = 0;
        }
      }
    }
